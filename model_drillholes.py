...
#legend{{position:fixed;right:10px;top:10px;background:#0f1724dd;color:#dce7ff;border:1px solid #2f3f60;border-radius:10px;padding:10px 12px;min-width:220px}}
...
<body>
<canvas id='c'></canvas>
<div id='legend'><div><b>Grade Legend (%Sb)</b></div><div><span class='sw' style='background:#4d7cff'></span>&lt; 1.0</div><div><span class='sw' style='background:#00c8ff'></span>1.0 - 4.99</div><div><span class='sw' style='background:#ff9a00'></span>5.0 - 9.99</div><div><span class='sw' style='background:#ff4d00'></span>10.0 - 19.99</div><div><span class='sw' style='background:#ff00ff'></span>20+</div><hr style='border-color:#324465'><div><span class='sw' style='background:#ffd100'></span>Proposed strike</div><div><span class='sw' style='background:#202020'></span>Drill trace</div></div>
<div id='hud'>
...
function drawLine(a,b,color,w){{ctx.strokeStyle=color;ctx.lineWidth=w;ctx.lineCap='round';ctx.shadowColor=color;ctx.shadowBlur=Math.max(0,w-1);ctx.beginPath();ctx.moveTo(a[0],a[1]);ctx.lineTo(b[0],b[1]);ctx.stroke();ctx.shadowBlur=0;}}
function render(){{
  canvas.width=innerWidth;canvas.height=innerHeight;const g=ctx.createLinearGradient(0,0,0,canvas.height);g.addColorStop(0,'#dff0ff');g.addColorStop(1,'#f4f7fb');ctx.fillStyle=g;ctx.fillRect(0,0,canvas.width,canvas.height);
  const drawItems=[];
  ...
  for(const it of drawItems){{
    if(it.type==='tri'){{
      ...
      ctx.fillStyle=`rgba(${{shade-20}},${{shade+10}},${{shade+25}},0.34)`;
